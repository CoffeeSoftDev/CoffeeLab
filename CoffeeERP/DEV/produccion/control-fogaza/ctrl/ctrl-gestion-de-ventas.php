...
function ticketMermas($obj){
    ...
    foreach ($lsAreas as $_key) {
        ...
        $__row[] = array(
            ...
            'Real'     => '',
            'precio'   => '',
            'total'    => '<strong>' . evaluar($totalArea) . '</strong>',
            'opc'      => 2,
        );
    }
...
function ls($obj,$th){
    ...
    $cont = count($ls);
        ...
        foreach ($ls as $key) {
            ...
            $btn[] = [
                "icon" => "  icon-lock-open-7",
                "color" => "info",
                "title" => 'Reabrir ticket',
                "fn" => "destajo.reabrirTicket"
            ];
