let ctrl_admin = "ctrl/ctrl-administracion.php";
...
$(function () {
    ...
    send_ajax(datos, ctrl_admin).then((data) => {
...
function rpt_detallado() {
    ...
    fn_ajax(dtx, ctrl_admin, "#tbDatos").then((data) => {
...
function enlace_receta(id, idHomologo) {
    ...
    <span class="text-info">${nombre}</span> con el producto ${txt} ? `,
        ...
        if (result.isConfirmed) {
            ...
            simple_send_ajax(datos, ctrl_admin).then((data) => { });
        }
    });
}
        ...
        success: function (data) {
            if (udn == 6) {
                ...
                $("#tbDatos").btn_json_table({
                    ...
                    right: [6],
                    th: "bg-primary",
                    id: "table-fogaza-export",
                    grupo: "bg-warning",
                });

                $("#txtTotals").html(data.Productos);

                datable_export_excel("#table-fogaza-export", 100);
            } else {
