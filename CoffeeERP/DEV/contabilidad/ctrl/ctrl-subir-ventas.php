    ...
    if($key['error'] == UPLOAD_ERR_OK ){
        ...
        switch ($nombre_archivo[0]) { //Recorrido por nombre de archivo 
            ...
            case 'VENTAS':
                ...
                $isCreatedFolio  =  $obj -> getCreatedFolio([ $id_folio ]);
                $data_ventas     = [];
                $data_ventas['noHabitaciones']  = $total_personas;
                $data_ventas['personas']  = $total_personas;

                if($isCreatedFolio) : // actualizar

                    $data_ventas['id_venta'] = $isCreatedFolio[0]['id_venta'];
                    $updt                    = $util->sql($data_ventas, 1);
                    $query                   = $obj ->actualizarVentas($updt);
                    ...
                    $updt                      = $util ->sql($data_ventas);
                    $query                     = $obj  ->add_Ventas($updt);
