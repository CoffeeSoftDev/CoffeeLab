...
class Costos extends MCostos {
    ...
    public function tbCostos(){
        ...
        foreach ($sqlUDN as $udn) {
            ...
            // Validar que sqlCuentas sea un array
            if (!is_array($sqlCuentas)) {
                $sqlCuentas = [];
            }
            ...
            if( $idE == 6 ) { //INFORMACION DE FOGAZA
                ...
                // Validar que sqlCuentas sea un array antes del segundo foreach
                if (is_array($sqlCuentas)) {
                    foreach ($sqlCuentas as $venta) {
                        $id                    = $venta['idUV'];
                        $sumaActual            = $this->sumaVentas([$idE,$id,$dates1[0],$dates1[1]]);
    ...
    function totalGeneral($dates1,$dates2){
        ...
        foreach ($udn as $idE) {
            $range1 = [$dates1[0],$dates1[1]];
            $range2 = [$dates2[0],$dates2[1]];

            switch ($idE) {
                case "1":
                        $ventaActual      += $this->ventaQT($range1);
                        $costoActual      += $this->costoQT($range1);
                        $ventaComparativa += $this->ventaQT($range2);
                        $costoComparativo += $this->costoQT($range2);
                    break;
                case "5":
                        $ventaActual      += $this->ventasSM($range1);
                        $costoActual      += $this->costoSM($range1);
                        $ventaComparativa += $this->ventasSM($range2);
                        $costoComparativo += $this->costoSM($range2);
                    break;
                case "4":
                case "7":
                        $ventaActual      += $this->ventaTotalUDN([$idE, $dates1[0], $dates1[1]]);
                ...
                case "6":
                        $ventaActual      += $this->ventaTotalUDN([$idE, $dates1[0], $dates1[1]]);
                        $ventaComparativa += $this->ventaTotalUDN([$idE, $dates2[0], $dates2[1]]);
                        $costoActual      += $this->costoMateriaPrimaFogaza($range1) + $this->costoManoObraFogaza($range1);
                        $costoComparativo += $this->costoMateriaPrimaFogaza($range2) + $this->costoManoObraFogaza($range2);
                    break;
            }
        }
        ...
        $porcentajeComparativo = $ventaComparativa != 0 ? ($costoComparativo / $ventaComparativa) * 100 : 0;
        $diferenciaPorcentaje  = $porcentajeActual- $porcentajeComparativo;
        $text                  = $diferenciaPorcentaje < 0 ? 'text-danger' : '';
