...
class ctrl extends mdl {
    ...
    public function apiPromediosDiarios() {
        ...
        return [
            ...
            'barDays'   => $this->apiIngresosComparativoSemana($anio1, $mes1, $anio2, $mes2, $udn),
    ...
    function getChequePromedioDailyByCategory() {
        $udn      = $_POST['udn']      ?? 1;
        ...
        if ($udn == 1) {
            $categorias = [
                'hospedaje' => ['label' => 'Hospedaje', 'color' => '#FF6384', 'field' => 'Hospedaje'],
                'ayb'       => ['label' => 'A&B', 'color' => '#36A2EB', 'field' => 'AyB'],
                'diversos'  => ['label' => 'Diversos', 'color' => '#FFCE56', 'field' => 'Diversos']
            ];
        } elseif ($udn == 5) {
            $categorias = [
                'alimentos'    => ['label' => 'Cortes', 'color' => '#FF6384', 'field' => 'alimentos'],
                'bebidas'      => ['label' => 'Bebidas', 'color' => '#36A2EB', 'field' => 'bebidas'],
                'guarniciones' => ['label' => 'Guarniciones', 'color' => '#FFCE56', 'field' => 'guarniciones'],
                'sales'        => ['label' => 'Sales', 'color' => '#4BC0C0', 'field' => 'sales'],
                'domicilio'    => ['label' => 'Domicilio', 'color' => '#9966FF', 'field' => 'domicilio']
            ];
        } else {
            $categorias = [
                'alimentos' => ['label' => 'Alimentos', 'color' => '#4CAF50', 'field' => 'alimentos'],
                'bebidas'   => ['label' => 'Bebidas', 'color' => '#2196F3', 'field' => 'bebidas']
            ];
        }
        ...
        for ($dia = 1; $dia <= $diasMes; $dia++) {
            ...
            $fechaObj = new DateTime($fecha);
            $diaSemana = $diasSemana[(int)$fechaObj->format('N')];
            $tooltip[] = $diaSemana . " " . $fechaObj->format('d');

            $clientes = isset($ventas['noHabitaciones']) ? (int)$ventas['noHabitaciones'] : 0;

            foreach ($categorias as $key => $config) {
                $field = $config['field'];
                $total = isset($ventas[$field]) ? (float)$ventas[$field] : 0;
        ...
        return [
            'status'   => 200,
            'message'  => 'Cheque promedio diario por categoría generado correctamente',
            'labels'   => $labels,
            'tooltip'  => $tooltip,
    ...
    function getChequePromedioByCategory() {
        ...
        if ($udn == 1) {
            ...
            $dataActual = [
                $clientesActual > 0 ? round(($ventasActual['totalHospedaje'] ?? 0) / $clientesActual, 2) : 0,
                $clientesActual > 0 ? round(($ventasActual['totalAyB'] ?? 0) / $clientesActual, 2) : 0,
                $clientesActual > 0 ? round(($ventasActual['totalDiversos'] ?? 0) / $clientesActual, 2) : 0
            ];

            $dataAnterior = [
                $clientesAnterior > 0 ? round(($ventasAnterior['totalHospedaje'] ?? 0) / $clientesAnterior, 2) : 0,
                $clientesAnterior > 0 ? round(($ventasAnterior['totalAyB'] ?? 0) / $clientesAnterior, 2) : 0,
                $clientesAnterior > 0 ? round(($ventasAnterior['totalDiversos'] ?? 0) / $clientesAnterior, 2) : 0
            ];
            ...
            $dataActual = [
                $clientesActual > 0 ? round(($ventasActual['totalAlimentos'] ?? 0) / $clientesActual, 2) : 0,
                $clientesActual > 0 ? round(($ventasActual['totalBebidas'] ?? 0) / $clientesActual, 2) : 0,
                $clientesActual > 0 ? round(($ventasActual['totalGuarniciones'] ?? 0) / $clientesActual, 2) : 0,
                $clientesActual > 0 ? round(($ventasActual['totalSales'] ?? 0) / $clientesActual, 2) : 0,
                $clientesActual > 0 ? round(($ventasActual['totalDomicilio'] ?? 0) / $clientesActual, 2) : 0
            ];

            $dataAnterior = [
                $clientesAnterior > 0 ? round(($ventasAnterior['totalAlimentos'] ?? 0) / $clientesAnterior, 2) : 0,
                $clientesAnterior > 0 ? round(($ventasAnterior['totalBebidas'] ?? 0) / $clientesAnterior, 2) : 0,
                $clientesAnterior > 0 ? round(($ventasAnterior['totalGuarniciones'] ?? 0) / $clientesAnterior, 2) : 0,
                $clientesAnterior > 0 ? round(($ventasAnterior['totalSales'] ?? 0) / $clientesAnterior, 2) : 0,
                $clientesAnterior > 0 ? round(($ventasAnterior['totalDomicilio'] ?? 0) / $clientesAnterior, 2) : 0
            ];
        ...
        return [
            'status'  => 200,
            'message' => 'Cheque promedio por categoría generado correctamente',
            'labels'  => $labels,
            'dataA'   => $dataAnterior,
            'dataB'   => $dataActual,
            'yearA'   => intval($anio2),
