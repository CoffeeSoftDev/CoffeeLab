...
class Perfil extends Templates {
    ...
    async renderPerfil() {
        ...
        if (this.dataUser.status == 200) {
            ...
            $('#phone').val(dataUsuario.data.phone || "No disponible");
            $('#birthday').val(dataUsuario.data.birthday || "No disponible");
    ...
    editUser() {
        ...
        if (!$('#fullname').val().trim() || !$('#user').val().trim() || !$('#phone').val().trim() || !$('#birthday').val().trim()) {
